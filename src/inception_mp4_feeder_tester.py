...
if __name__ == "__main__":
	...
	parser.add_argument("-t","--timestamp",default=1,help="The elapsed time between each detection update",type=int)
	parser.add_argument("-r","--imgRate",default=30,help="The image rate of the given mp4",type=int)
	parser.add_argument("-a","--alpha",default="1.5",help="Size factor by which the patch is reduce at each iteration",type=float)
	...
	image_butcher = ImageButcher(args.alpha,args.patch_factor)
		...
		for z in range(len(pred)):
			for y in range(len(pred[z])):
				for x in range(len(pred[z][0])):
					...
					metadata = image_butcher.get_metadata(img,x,y,z)
